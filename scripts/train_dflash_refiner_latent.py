#!/usr/bin/env python3
# coding=utf-8
"""CO-TRAIN the LATENT-JACOBI AR refiner together with the DFlash drafter backbone.

SEPARATE script (does not modify the cotrain trainer). The refiner iterates in HIDDEN space:
the `prev` input is the previous-position hidden (not embed(prev_token)), and training UNROLLS
K latent-Jacobi passes with a per-pass consistency loss + detached prev (see dflash_refiner_latent.py).
Goal: at inference the K passes feed hidden forward and lm_head runs ONCE at the end (K+1 -> 1).

Differences vs the token co-train trainer:
  - model is `OnlineDFlashRefinerLatentCoTrain` (latent head + unrolled-Jacobi consistency forward);
  - extra knobs: --latent-k, --latent-k-sample, --no-latent-prev-norm, --latent-loss;
  - everything else (CoTrainFeatureExtractor, FSDP, base_loss + lambda_base anchor, optimizer,
    per-rank resume, checkpoint draft_state_dict + refiner_state_dict) is unchanged.
"""
...
from specforge.core.dflash_refiner_latent import (
    OnlineDFlashRefinerLatentCoTrain,
)
...
def parse_args():
    ...
    latent_group = parser.add_argument_group("latent jacobi")
    latent_group.add_argument(
        "--latent-k", type=int, default=3,
        help="number of unrolled latent-Jacobi passes trained per step (fixed; match inference par-K).",
    )
    latent_group.add_argument(
        "--no-latent-prev-norm", action="store_true",
        help="disable the RMSNorm on the prev hidden (default: on).",
    )
    latent_group.add_argument(
        "--latent-loss", type=str, default="uniform", choices=["uniform", "final_heavy"],
        help="per-pass consistency-loss weighting: 'uniform' (all passes equal) | "
        "'final_heavy' (weight pass t by t+1, emphasize the converged output).",
    )
        ...
        torch.save(
            {
                ...
                "latent_prev_norm": not args.no_latent_prev_norm,
...
def main():
    ...
    refiner_model = OnlineDFlashRefinerLatentCoTrain(
        feature_extractor,
        window_size=args.window_size,
        num_refiner_layers=args.num_refiner_layers,
    ...
    if int(os.environ.get("RANK", "0")) == 0:
        print(
            ...
            f"  LATENT JACOBI: k={args.latent_k} (fixed)  "
            f"prev_norm={not args.no_latent_prev_norm} loss={args.latent_loss}\n"
